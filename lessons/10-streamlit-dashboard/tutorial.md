...
This is the written companion to Lesson 10. The lesson runs in one class session plus a short take-home that feeds Milestone 02:

- **In-class (Wed Apr 29):** Build and deploy a Streamlit dashboard for Maya, the Head of Merchandising you designed the dbt mart for in MP02 Step 25. Steps 00–07. Goal: leave class with a public Streamlit Community Cloud URL.
- **Take-home (Apr 29 → May 6):** Produce your portfolio pipeline diagram (M02 #9). Step 08.

## What is Streamlit?

[Streamlit](https://streamlit.io) is an open-source Python framework that turns a Python script into an interactive web app. You write Python — `import streamlit as st`, then describe the layout and charts you want — and Streamlit handles the HTML, the widget reactivity, the styling, and the page rendering. No JavaScript, no separate front end, no template engine. For data dashboards backed by a warehouse, it's the fastest path from a SQL query to something a stakeholder can click.

Today you'll write about a hundred lines of Python (most of it generated by Claude Code) and end up with a public-URL dashboard Maya could actually use. Streamlit Community Cloud, the free hosting platform we'll deploy to in Step 07, reads directly from your GitHub repo and rebuilds on every push.

## Who you're building for

Maya, the Head of Merchandising at Basket Craft, is your stakeholder. The dbt mart you built in MP02 was designed around her questions:
...
The dashboard you build today answers her first two questions directly. The KPIs and trend chart give her the headline; the product bar chart shows what's selling; the product drill-down surfaces co-purchase patterns she can use to design bundles.
...
**Reading top to bottom:** what's happening overall (KPIs) → how revenue moved over time (trend) → which products are driving it (top products) → which products go together (bundle finder). Descriptive at top, diagnostic in the middle, actionable at the bottom — that's the M02 #7 rubric. The bundle finder is downloadable CSV; that's what Maya hands to a buyer to design promotions.

**Before class.** Confirm your MP02 Snowflake account is still active and your basket_craft star schema is still in the `analytics` schema. Log into Snowsight the night before and run a quick `SELECT` on a dimension table to verify.
...
2. Ask Claude Code to run it:
   ...
   A browser tab opens at `localhost:8501` showing one number — your product-dimension row count.
...
The dashboard has four sections, walking the analytical arc top to bottom: KPIs (what's happening) → trend (how we got here) → top products (which products are driving it) → bundle finder (which products go together). Same descriptive-then-diagnostic split the M02 rubric grades on. Every section answers one of Maya's MP02 questions.
